...
//    $outer_criteria = 'b.biblio_id > 0 ';
    ...
    $xlsquery = 'select b.title Title, f.file_title `File title`, f.mime_type Type, count(fr.filelog_id) Access from search_biblio b left join biblio_attachment ba on b.biblio_id = ba.biblio_id left join files f on f.file_id = ba.file_id left join files_read fr on fr.file_id = f.file_id WHERE '. $criteria . ' group by f.file_id';
        // echo $xlsquery;
