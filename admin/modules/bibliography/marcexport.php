...
if (!class_exists('File_MARC')) {
  die('<div class="errorBox">'.__('<a href="http://pear.php.net/index.php">PEAR</a>, <a href="http://pear.php.net/package/File_MARC">File_MARC</a>
    packages need to be installed in order
      to export MARC record').'</div>');
  }
  
  function marc_array($input_id = 0) {
    global $dbs;
    // require 'File/MARC.php';
    $marc_records = '';
    if (!$input_id) {
      $rec_bib_q = $dbs->query('SELECT s.*, b.collation FROM search_biblio AS s LEFT JOIN biblio AS b ON b.biblio_id = s.biblio_id LIMIT 10000');
    } else {
      $rec_bib_q = $dbs->query(sprintf('SELECT s.*, b.collation FROM search_biblio AS s
      LEFT JOIN biblio AS b ON b.biblio_id = s.biblio_id WHERE s.biblio_id IN (%s)', $input_id));
    }
