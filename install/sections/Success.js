...
export default {
    ...
    computed: {
        hasUpgradeMessages() {
            return this.optionalmsg.length > 0
        }
    },
    template: `
<div class="h-screen flex">
    ...
    <div class="flex-1 bg-gray-100 py-8 px-16">
        ...
        <p class="rounded border bg-pink-100 border-pink-500 text-pink-500 px-4 py-2 mt-2 mb-8 text-lg">
            Folder <code class="font-bold px-2">install</code> in your <slims-text></slims-text> is already exist.
            For security reason please rename or remove it from your machine.
        </p>

        <div v-if="hasUpgradeMessages">
            <h1 class="text-3xl font-medium">Upgrade result</h1>
            <p class="mb-4 mt-1 text-grey-700 leading-normal">
                Some upgrade process didn't work, but don't worry it doesn't have big impact on your <slims-text></slims-text>,
                <strong class="font-bold cursor-pointer" title="For more information" @click="showDetail = !showDetail">show detail</strong>.
                Now it ready to use.
            </p>
            <div v-if="showDetail" class="rounded border bg-blue-100 border-blue-500 text-blue-500 px-4 py-2 mt-2 mb-8 text-lg">
                <p v-for="msg in optionalmsg" :key="msg">{{ msg }}</p>
            </div>
        </div>
        ...
        <div class="w-2/3">
            ...
            <p class="mb-4 mt-1 text-grey-700 leading-normal">
                Support <slims-text></slims-text> development and become part of its history.
                We appreciate every donation to support us in any way. If you willing to make donation, please follow this link:
